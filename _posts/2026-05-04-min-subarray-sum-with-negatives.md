...
tags: [prefix-sum, sliding-window, array, interview, leetcode, deque, hard]
description: "Prefix Sums vs Sliding Window: What If LC 209 Had Negative Numbers?"
---
...
A deque is ideal here because we need O(1) access to both ends: the **front** to check validity against the current `end`, and the **back** to maintain the monotonic invariant as we add new candidates.
 
We maintain a deque of candidate `start` indices such that their prefix values are **strictly increasing**. This guarantees:
...
1. **Check validity** (front): While `prefix[end] - prefix[deque.front] ≥ target`, we found a valid subarray. Pop front and update answer. Popping greedily gives the shortest length for the current `end`.
2. **Maintain monotonicity** (back): While `prefix[deque.back] ≥ prefix[end]`, pop back. A newer index with a smaller or equal prefix value always dominates — it can only yield a shorter subarray.
...
public class MinSubarrayWithNegatives {
    ...
    public int minSubArrayLen(int target, int[] nums) {
        ...
        // Step 1: Build prefix sum array using long to avoid integer overflow
        ...
        for (int end = 0; end <= n; end++) {
            // Step 2: Pop from front while the subarray [front..end] is valid
            // Greedy: front gives the smallest prefix, so end - front is shortest
            while (!deque.isEmpty() && prefix[end] - prefix[deque.peekFirst()] >= target) {
                answer = Math.min(answer, end - deque.pollFirst());
            }
 
            // Step 3: Pop from back to maintain strictly increasing prefix values
            // A newer index with a smaller prefix always dominates the old one
            while (!deque.isEmpty() && prefix[deque.peekLast()] >= prefix[end]) {
                deque.pollLast();
            }
 
            // Step 4: Add current index as a future candidate start
            deque.offerLast(end);
        }
